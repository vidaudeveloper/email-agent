...
**Zero-dependency ESP sync (when Resend is the ESP)**: `python3 "/Users/kean/Desktop/DemoFile/email_demo/scripts/connectors/resend.py" contacts` / `segments` reads the live roster and segment list, and — after the suppression is recorded in the consent-registry — `resend.py suppress <id-or-email> --live` pushes it to the platform (`unsubscribed: true`). The registry stays the SSOT; Resend is a downstream mirror. Mutating subcommands are dry-run by default (`--live` to execute). See [scripts/connectors/README.md](../../../scripts/connectors/README.md).
